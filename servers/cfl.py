...
class ClusterFL(Server):
    ...
    def train(self):
        new_attend_clients = []
        slogger.debug('-'*20,'server ClusterFL starting to train','-'*20)
    ...
    def fine_tuning_new_clients(self,new_clients):
        for new_client in new_clients:
            ...
            slogger.info('cluster close to {} is cluster {}'.format(new_client.id,close_cluster_id))
    ...
    def compute_pairwise_similarities(self, clients):
        ...
        slogger.info('compute pairwise similarities time:{:2f}'.format(time.time()-start_time))
        return angles
